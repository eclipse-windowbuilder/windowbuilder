...
public class FormLayoutEditPolicyClassic<C extends IControlInfo>
  ...
  protected void decorateChild(EditPart child) {
    if (layout.getControls().contains(child.getModel())) {
      child.installEditPolicy(
          ...
          new FormSelectionEditPolicyClassic<C>(layout));
    }
  }
  ...
  private void showMoveFeedback(ChangeBoundsRequest request, GraphicalEditPart part)
      throws Exception {
  ...
  private void showMoveFeedback(GraphicalEditPart part, Rectangle bounds) throws Exception {
    ...
    IFormAttachmentInfo<C> leftAttachment =
        layoutImpl.getAttachment(control, IPositionConstants.LEFT);
    IFormAttachmentInfo<C> rightAttachment =
        layoutImpl.getAttachment(control, IPositionConstants.RIGHT);
    IFormAttachmentInfo<C> topAttachment =
        layoutImpl.getAttachment(control, IPositionConstants.TOP);
    IFormAttachmentInfo<C> bottomAttachment =
        layoutImpl.getAttachment(control, IPositionConstants.BOTTOM);
    // hide any old move feedbacks (it is more easy to add each time new feedback than track old ones and change their positions)
  ...
  private void updateTextFeedback(TextFeedback feedback,
      String text,
      Rectangle bounds,
      Dimension parentSize_,
      boolean isLeading,
  ...
  //
  ////////////////////////////////////////////////////////////////////////////
  private String showBottomSideFeedbacks(GraphicalEditPart part,
      C control,
      Rectangle bounds,
      String locationY,
      IFormAttachmentInfo<C> bottomAttachment,
    ...
    addVLine(
        part,
        bounds.bottom(),
        layout.getComposite().getClientArea().width,
        IColorConstants.red);
    if (createControl) {
  ...
  private String showRightSideFeedbacks(GraphicalEditPart part,
      C control,
      Rectangle bounds,
      String locationX,
      IFormAttachmentInfo<C> rightAttachment,
    ...
    addHLine(
        part,
        bounds.right(),
        layout.getComposite().getClientArea().height,
        IColorConstants.red);
    if (createControl) {
      createCommand.add(cmd);
    } else {
  ...
  private String showTopSideFeedbacks(final GraphicalEditPart part,
      C control,
      Rectangle bounds,
      Point location,
      String locationY,
  ...
  private String showLeftSideFeedbacks(final GraphicalEditPart part,
      C control,
      final Rectangle bounds,
      final Point location,
      String locationX,
      final IFormAttachmentInfo<C> leftAttachment,
      final List<C> attachables,
      boolean createControl) {
    // prepare values from preferences
    int layoutMarginLeft = FormUtils.getLayoutMarginLeft(layout);
  ...
  private Command getBindToControlCommand(boolean createControl,
      C control,
      int controlSide,
      C child,
      int cmdSide,
  ...
  private void showSelectionFeedbacks() {
    for (EditPart child : getHost().getChildren()) {
      if (layout.getControls().contains(child.getModel())
        ...
        FormSelectionEditPolicyClassic<C> editPolicy =
            (FormSelectionEditPolicyClassic<C>) child.getEditPolicy(EditPolicy.SELECTION_ROLE);
  ...
  protected TextFeedback getTextFeedback(Map<EditPart, TextFeedback> map,
      EditPart key,
      boolean isHorizontal) {
