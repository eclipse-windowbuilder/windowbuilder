...
public final class EditSelection {
	...
	private static void setToPage(IDatabindingsProvider databindingsProvider,
		...
		page.masterSelection = objectToPath(masterProvider, masterInput, observeElementsComposite.getMasterObserve());
	 ...
	 */
	public boolean setSelection(IDatabindingsProvider databindingsProvider, BindingElementsComposite bindingComposite,
			ObserveElementsComposite targetComposite, ObserveElementsComposite modelComposite) {
		...
		if (m_currentBindingIndex > -1 && m_currentBindingIndex < bindings.size()) {
			bindingComposite.getViewer().setSelection(new StructuredSelection(bindings.get(m_currentBindingIndex)),
					true);
			result = true;
		} else {
	...
	private static boolean setPageSelection(IDatabindingsProvider databindingsProvider,
		...
		try {
			...
			Object propertiesSelection = pathToObject(propertiesProvider, propertiesInput, page.propertiesSelection);
			...
			Object[] propertiesExpanded = pathsToObjects(propertiesProvider, propertiesInput, page.propertiesExpanded);
