...
/**
 * {@link ChooseClassUiContentProvider} with {@link CheckboxTableViewer} for
 * represented properties for chosen classes.
 *
 * @author lobas_av
...
public abstract class ChooseClassAndPropertiesUiContentProvider extends ChooseClassUiContentProvider {
	...
	public void createContent(Composite parent, int columns) {
		...
		if (m_configuration.isReorderMode()) {
			...
			configureDND();
			m_propertiesViewer.getViewer().addPostSelectionChangedListener(event -> calculateUpDowButtons());
			// grid filler
	...
	/**
	 * Select given class name and property. If <code>className</code> is't class
	 * (maybe special default value) sets <code>loadClass</code> as
	 * <code>false</code>.
	 */
	 ...
	 */
	protected abstract void saveToObject(Class<?> choosenClass, List<PropertyAdapter> choosenProperties)
			throws Exception;
}