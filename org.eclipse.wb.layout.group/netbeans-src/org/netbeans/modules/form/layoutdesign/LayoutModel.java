...
/**
 * This class manages layout data of a form. Specifically it: - provides entry points for exploring
...
public class LayoutModel implements LayoutConstants {
  ...
  private final Map<String, LayoutComponent> idToComponents =
      new HashMap<String, LayoutComponent>();
  ...
  private final Map<Integer, LayoutEvent> undoMap = new HashMap<Integer, LayoutEvent>(500);
  private final Map<Integer, LayoutEvent> redoMap = new HashMap<Integer, LayoutEvent>(100);
  ...
  void unregisterComponentImpl(LayoutComponent comp) {
    ...
    if (lc != null) {
      // record undo/redo and fire event
      LayoutEvent.Component ev =
          new LayoutEvent.Component(this, LayoutEvent.COMPONENT_UNREGISTERED);
      ev.setComponent(comp);
      addChange(ev);
      fireEvent(ev);
    } // else noop => don't need change event
  ...
  public void addComponent(LayoutComponent component, LayoutComponent parent, int index) {
  ...
  void changeIntervalAttribute(LayoutInterval interval, int attribute, boolean set) {
    ...
    // record undo/redo (don't fire event)
    LayoutEvent.Interval ev =
        new LayoutEvent.Interval(this, LayoutEvent.INTERVAL_ATTRIBUTES_CHANGED);
  ...
  void setIntervalAlignment(LayoutInterval interval, int alignment) {
    ...
    // record undo/redo (don't fire event)
    LayoutEvent.Interval ev =
        new LayoutEvent.Interval(this, LayoutEvent.INTERVAL_ALIGNMENT_CHANGED);
  ...
  void setLayoutContainer(LayoutComponent component, boolean container) {
    ...
    if (oldContainer != container) {
      ...
      // record undo/redo (don't fire event)
      LayoutEvent.Component ev =
          new LayoutEvent.Component(this, LayoutEvent.CONTAINER_ATTR_CHANGED);
  ...
  public void setPaddingType(LayoutInterval interval, PaddingType paddingType) {
    ...
    if (oldPadding != paddingType) {
      ...
      // record undo/redo (don't fire event)
      LayoutEvent.Interval ev =
          new LayoutEvent.Interval(this, LayoutEvent.INTERVAL_PADDING_TYPE_CHANGED);
   ...
   *          mapping between the original and the copied components' IDs
   * @param targetContainerId
   *          ID of the target container
   */
  public void copyContainerLayout(LayoutModel sourceModel,
  ...
  void moveContainerLayout(LayoutComponent sourceContainer, LayoutComponent targetContainer) {
    if (!sourceContainer.isLayoutContainer()
        || !targetContainer.isLayoutContainer()
  ...
  private static class RegionInfo {
    ...
    public void calculateIntervals() {
      ...
      do {
        boolean remove = dimension == -1 && effDim == HORIZONTAL || dimension != -1 && effDim != -1;
        if (remove) {
          effDim = -1;
        }
        if (dimension == -1) {
    ...
    private List<Map<LayoutComponent, Rectangle>> cutIntoParts(Set<Integer> cutSet, int dimension) {
      List<Map<LayoutComponent, Rectangle>> parts =
          new LinkedList<Map<LayoutComponent, Rectangle>>();
      ...
      while (iter.hasNext()) {
        ...
        Map<LayoutComponent, Rectangle> postCompToBounds =
   ...
   * @param dimension
   *          dimension in which the layout interval resides.
   */
   ...
   */
  public String saveContainerLayout(LayoutComponent container,
      Map<String, String> idToNameMap,
      int indent,
      boolean humanReadable) {
    return LayoutPersistenceManager.saveContainer(
        this,
        container,
        idToNameMap,
        indent,
   ...
   */
  public void loadContainerLayout(String containerId,
      org.w3c.dom.NodeList layoutNodeList,
  ...
  private final Map<Integer, List<String>> linkSizeGroupsV = new HashMap<Integer, List<String>>();
  ...
  void addComponentToLinkSizedGroupImpl(int groupId, String compId, int dimension) {
    ...
    // record undo/redo and fire event
    LayoutEvent.Component ev =
        new LayoutEvent.Component(this, LayoutEvent.INTERVAL_LINKSIZE_CHANGED);
  ...
  /**
   * @return returns FALSE if components are not linked, and if so, they are linked in the same
   *         group returns TRUE if all components are in the same linksize group returns INVALID if
