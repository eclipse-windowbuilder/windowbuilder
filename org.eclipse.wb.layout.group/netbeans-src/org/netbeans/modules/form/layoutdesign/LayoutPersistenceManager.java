...
class LayoutPersistenceManager implements LayoutConstants {
   ...
   * @param idToNameMap
   *          map for translating component Ids to names suitable for saving
   * @param indent
   *          determines size of indentation
   * @param humanReadable
   *          determines whether constants should be replaced by human readable expressions
   * @return the layout model saved in a String
   */
  static String saveContainer(LayoutModel layoutModel,
  ...
  private String saveLayout() {
    ...
    for (dimension = 0; dimension < DIM_COUNT; dimension++) {
      ...
      if (humanReadable) {
        switch (dimension) {
          case HORIZONTAL :
            sb.append(VALUE_DIMENSION_HORIZONTAL);
            break;
          case VERTICAL :
            sb.append(VALUE_DIMENSION_VERTICAL);
            break;
          default :
            sb.append(dimension);
            break;
   ...
   * @param interval
   *          the layout interval that should be dumped
   * @param dimension
   *          the dimension where the interval belongs
   * @param indent
   *          determines size of indentation
   * @return dump of the layout model
   */
  static String dumpInterval(LayoutModel layoutModel,
   ...
   *          alignment to save.
   * @param group
   *          determines whether it is a group alignment.
   */
      ...
      if (alignment != DEFAULT) {
        ...
        switch (alignment) {
          case LEADING :
            ...
            break;
          case TRAILING :
            sb.append(VALUE_ALIGNMENT_TRAILING);
            break;
          case CENTER :
            sb.append(VALUE_ALIGNMENT_CENTER);
            break;
          case BASELINE :
            sb.append(VALUE_ALIGNMENT_BASELINE);
            break;
          default :
            assert false;
        }
   ...
   *          XML data to load
   * @param nameToIdMap
   *          map from component names to component IDs
   */
  static void loadContainer(LayoutModel layoutModel,
      String containerId,
      NodeList layoutNodeList,
      Map<String, String> nameToIdMap) throws java.io.IOException {
    LayoutPersistenceManager lpm = new LayoutPersistenceManager(layoutModel);
  ...
  private void loadLayout(String containerId, NodeList layoutNodeList) throws java.io.IOException {
   ...
   * @param groupNode
   *          node holding the information about the layout of the group.
   */
  private void loadGroup(LayoutInterval group, Node groupNode) throws java.io.IOException {
    ...
    if (group.isParallel()) {
      int groupAlignment =
          groupAlignmentNode == null ? DEFAULT : integerFromNode(groupAlignmentNode);
   ...
   * @param spaceNode
   *          node with the information about the empty space.
   */
   ...
   * @param componentNode
   *          node with the information about the component.
   */
  private void loadComponent(LayoutInterval parent, Node componentNode) throws java.io.IOException {
    ...
    if (linkSizeId != null) {
      layoutModel.addComponentToLinkSizedGroup(
          integerFromNode(linkSizeId),
          layoutComponent.getId(),
          dimension);
    }
   ...
   * @param attrMap
   *          map with attribute information.
   */
  ...
  // error recovery
  /**
   * This method is used during loading to check the alignment validity of given group and its
   * subintervals. It checks use of BASELINE alignment of the group and the subintervals. Some
   * invalid combinations were allowed by GroupLayout in version 1.0. See issue 78035 for details.
   * This method also fixes the invalid combinations and sets the 'corrected' flag. This is needed
   * for loading because wrong layouts still might exist from the past (and we still can't quite
   * exclude it can't be created even now). BASELINE group can only contain BASELINE intervals, and
   * vice versa, BASELINE interval can only be placed in BASELINE group. BASELINE can be set only on
   * individual components. LEADING, TRAILING and CENTER alignments can be combined freely.
   */
  ...
  private void correctMissingName() throws java.io.IOException {
    ...
    if (missingNameH != null
        && missingNameV != null
        && missingNameH.equals(missingNameV)
        && idNameMap.size() == layoutContainer.getSubComponentCount()) { // we have one unknown name in each dimension, let's infer it from the idNameMap
