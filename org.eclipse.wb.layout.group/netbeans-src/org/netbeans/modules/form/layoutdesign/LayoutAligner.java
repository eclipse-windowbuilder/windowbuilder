...
class LayoutAligner implements LayoutConstants {
	 ...
	 *
	 * @param designer
	 *          layout designer to use.
	 * @param layoutModel
	 *          layout model to use.
	 * @param operations
	 *          layout operations to use.
	 */
	...
	void alignIntervals(LayoutInterval[] intervals, boolean closed, int dimension, int alignment) {
		...
		List<LayoutInterval> removedSeqs = new LinkedList<>();
		...
		if (alignment != CENTER) {
			...
			Map<LayoutInterval, LayoutInterval> leadingMap =
					new HashMap<>();
			Map<LayoutInterval, LayoutInterval> trailingMap =
					new HashMap<>();
			...
			if (closed) {
				...
				LayoutInterval[] itervalsToAlign =
						alignment == LEADING ? leadingIntervals : trailingIntervals;
	...
	/**
	 * Marks layout intervals by <code>ATTR_ALIGN_PRE</code> and <code>ATTR_ALIGN_POST</code>
	 * attributes. The first one is assigned to the intervals that are in front of some from the
	 * passed <code>intervals</code>. The second one is assigned to the intervals that are behind some
	 * of the passed <code>intervals</code>. Note that some intervals may obtain both of them.
	 *
	 * @param parParent
	 *          parallel parent of all passed intervals (the marking is restricted to this interval).
	 * @param intervals
	 *          intervals according which the marking is done.
	 */
	...
	/**
	 * Marks the <code>interval</code> by the given attribute. If the interval is group, then all
	 * subintervals are marked as well.
	 *
	 * @param interval
	 *          interval to mark.
	 * @param attr
	...
	private LayoutInterval splitByAlignAttrs(LayoutInterval interval,
			List<LayoutInterval> removedSeqs,
			int alignment,
			boolean closed,
			int dimension,
			boolean optimize) {
							...
							while (subIter.hasNext()) {
								...
								if (preSubSub) {
									if (postSubSub) {
										...
										if (!subSubInterval.isEmptySpace()
												|| subSubInterval.getPreferredSize() == NOT_EXPLICITLY_DEFINED) {
											trailingPre = subSubInterval;
										}
									}
								} else if (postSubSub) {
									if (leadingPost == null
											&& (!subSubInterval.isEmptySpace()
													|| subSubInterval.getPreferredSize() == NOT_EXPLICITLY_DEFINED)) {
					...
					for (int i = interval.getSubIntervalCount() - 1; i >= 0; i--) {
						...
						if (subInterval.isSequential()) {
							...
							if (alignment == LEADING) {
								...
								if (lastPreGap != null) {
									int delta = shortenGap(
											lastPreGap,
											getPosition(firstMid, dimension, LEADING) - minMid - shift);
								...
								if (firstPostGap != null) {
									int delta = shortenGap(
											firstPostGap,
											maxMidWidth - (getPosition(lastMid, dimension, TRAILING) - minMid) + shift);
							...
							if (alignment == TRAILING) {
								...
								if (firstPostGap != null) {
									int delta = shortenGap(
											firstPostGap,
											maxMid - getPosition(lastMid, dimension, TRAILING) - shift);
									leading[subInterval.indexOf(firstPostGap)] += delta;
								}
								...
								if (lastPreGap != null) {
									int delta = shortenGap(
											lastPreGap,
											maxMidWidth - (maxMid - getPosition(firstMid, dimension, LEADING)) + shift);
									trailing[subInterval.indexOf(lastPreGap)] -= delta;
								}
							}
	...
	private LayoutInterval putGroupToGroup(LayoutInterval groupToInsert,
	 ...
	 *
	 * @param interval
	 *          interval whose current space should be expanded.
	 * @param dimension
	 *          dimension in which the expansion occurs.
	 * @param leading
	 *          lower bound of the expansion.
	 * @param trailing
	 *          upper bound of the expansion.
	 */
	private void expandCurrentSpace(LayoutInterval interval,
			int dimension,
			int leading,
			int trailing) {
	 ...
	 *
	 * @param intervals
	 *          intervals that should be transfered into the given parallel parent.
	 * @param parParent
	 *          parallel group that is already parent (but maybe not the nearest parallel parent) of
	 *          the given intervals.
	 * @param requested
	 *          alignment.
	...
	private List<LayoutInterval> transferToParallelParent(LayoutInterval[] intervals,
		...
		List<LayoutInterval> gapsToResize = new LinkedList<>();
		...
		List<List<LayoutInterval>> newSequences = new LinkedList<>();
		Map<LayoutInterval, Integer> gapSizes = new HashMap<>();
		for (int i = 0; i < intervals.length; i++) {
			LayoutInterval interval = intervals[i];
			...
			List<LayoutInterval> newSequenceList = new LinkedList<>();
			...
			if (alignment == TRAILING && !closed) {
				...
				LayoutInterval gapInterval =
						LayoutInterval.getNeighbor(leadingInterval, SEQUENTIAL, LEADING);
				leadingGaps = leadingGaps && preGap != 0;
				if (gapInterval != null
						&& gapInterval.isEmptySpace()
						&& parParent.isParentOf(gapInterval)
					...
					gapSizes.put(gap, Integer.valueOf(preGap));
					if (alignment == TRAILING) {
			...
			while (iter.hasNext()) {
				...
				LayoutInterval gapInterval =
						LayoutInterval.getNeighbor(leadingInterval, SEQUENTIAL, TRAILING);
				...
				if (gapInterval.isEmptySpace()) {
					...
					if (neighbor == trailingInterval) {
						...
						LayoutInterval gap = cloneGap(gapInterval);
						newSequenceList.add(gap);
						gapSizes.put(gap, Integer.valueOf(gapSize));
			...
			if (alignment == LEADING && !closed) {
				...
				if (gapInterval != null
					...
					LayoutInterval gap = cloneGap(gapInterval);
					newSequenceList.add(gap);
					gapSizes.put(gap, Integer.valueOf(postGap));
					if (alignment == LEADING) {
						sequenceResizable = sequenceResizable || LayoutInterval.canResize(gap);
					}
				} else {
	...
	/**
	 * PENDING Determines layout components that will be transfered to the specified parallel parent
	 * together with the given layout component.
	 *
	 * @param interval
	 *          layout component to transfer to the parallel parent.
	 * @param parParent
	 *          parallel parent to transfer the component to.
	 * @return <code>List</code> of <code>LayoutInterval</code> objects.
	 */
	private List<LayoutInterval> transferedComponents(LayoutInterval[] intervals,
			int index,
			LayoutInterval parParent) {
		LayoutInterval interval = intervals[index];
		LayoutInterval oppInterval = oppositeComponentInterval(interval);
		List<LayoutInterval> transferedComponents = new LinkedList<>();
		List<LayoutInterval> components = new LinkedList<>();
		...
		Collections.sort(transferedComponents, new Comparator<LayoutInterval>() {
			...
			public int compare(LayoutInterval interval1, LayoutInterval interval2) {
				...
				int dimension = comp.getLayoutInterval(VERTICAL) == interval1 ? VERTICAL : HORIZONTAL;
	...
	/**
	 * Places all components that are in the <code>group</code> into <code>components</code>
	 * collection.
	 *
	 * @param group
	 *          layout interval that is scanned for components.
	 * @param components
	...
	/**
	 * Aligns given intervals to a parallel group. The intervals are supposed to have the same first
	 * parallel parent.
	 */
	private boolean align(LayoutInterval[] leadingInts,
			LayoutInterval[] trailingInts,
			boolean closed,
			int dimension,
			int alignment) {
		...
		for (int i = 0; i < intervals.length; i++) {
			...
			if (parent.isSequential()) {
				...
				int extractCount = operations.extract(
						interval,
						closed ? trailingInts[i] : interval,
								alignment,
								closed,
								restLeading,
								restTrailing);
		...
		if (!remainder && mainEffectiveAlign == alignment
				|| aligned.size() == originalCount && commonGroup.getParent() != null) { // reuse the original group - avoid unnecessary nesting
			group = commonGroup;
		...
		if (!restLeading.isEmpty()) {
			...
			designer.createRemainderGroup(
					restLeading,
					commonSeq,
					commonSeq.indexOf(group),
					LEADING,
					mainEffectiveAlign,
					dimension);
		}
	...
	/**
	 * Helper method that adds layout interval (empty space) to the given list (when the specified
	 * size is positive).
	 *
	 * @param list
	 *          list the gap should be added to.
	 * @param size
	...
	private static boolean alignedIntervals(LayoutInterval interval1,
	...
	private static boolean hasAlignmentInParent(LayoutInterval interval, int alignment) {
		...
		if (parent.isSequential()) {
			...
			assert interval.getAlignment() != alignment
					|| compatibleGroupAlignment(parent.getGroupAlignment(), alignment);
			return interval.getAlignment() == alignment || LayoutInterval.wantResize(interval);
		}
	}

	private static LayoutInterval getAlignSubstitute(LayoutInterval toAlignWith,
			LayoutInterval commonParParent,
			int alignment) {
		assert alignment == LEADING || alignment == TRAILING;
		while (toAlignWith != null
				&& LayoutInterval.getFirstParent(toAlignWith, PARALLEL) != commonParParent) {
	...
	/**
	 * Returns layout interval for the opposite dimension for the given layout interval of a layout
	 * component.
	 *
	 * @param interval
	 *          layout interval of some layout component.
	 * @return layout interval for the opposite dimension for the given layout interval of a layout
	 *         component.
	 */
	private LayoutInterval oppositeComponentInterval(LayoutInterval interval) {
		assert interval.isComponent();
		LayoutComponent component = interval.getComponent();
		int oppDimension = component.getLayoutInterval(HORIZONTAL) == interval ? VERTICAL : HORIZONTAL;
	...
	private void returnRemovedIntervals(LayoutInterval parParent,
		...
		while (iter.hasNext()) {
			...
			int pre = Math.max(
					0,
					region.positions[dimension][LEADING] - parRegion.positions[dimension][LEADING]);
			int post = Math.max(
					0,
					parRegion.positions[dimension][TRAILING] - region.positions[dimension][TRAILING]);
			...
			if (pre != 0) {
				LayoutInterval gap = new LayoutInterval(SINGLE);
				gap.setSize(pre);
				if (interval.getAlignment() == TRAILING) {
					designer.setIntervalResizing(gap, true);
				}
			...
			if (post != 0) {
				LayoutInterval gap = new LayoutInterval(SINGLE);
				gap.setSize(post);
				if (interval.getAlignment() == LEADING) {
					designer.setIntervalResizing(gap, true);
				}
	...
	private void setAlignmentAccordingEffectiveAlignment(LayoutInterval aligned,
