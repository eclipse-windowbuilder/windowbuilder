...
public interface LayoutConstants {
	// size constants
	/**
	 * Indicates a size or position not defined in the layout model; it must be obtained elsewhere
	 * (e.g. from a real component).
	 */
	int NOT_EXPLICITLY_DEFINED = -1;
	/**
	 * Specifies that a min or max size value should be the same as the preferred size value.
	 */
	...
	final PaddingType[] PADDINGS = {
			PaddingType.RELATED,
			PaddingType.UNRELATED,
			PaddingType.INDENT,
	...
	int SINGLE = 101;
	/**
	 * Indicates a layout interval containing a sequence of sub-intervals (placed one after anoother).
	 */
	int SEQUENTIAL = 102;
	/**
