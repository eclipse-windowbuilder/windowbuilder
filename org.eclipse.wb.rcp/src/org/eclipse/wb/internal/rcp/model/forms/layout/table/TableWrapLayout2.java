...
/**
 * This implementation of the layout algorithm attempts to position controls in the composite using
 * a two-pass autolayout HTML table altorithm recommeded by HTML 4.01 W3C specification (see
 * http://www.w3.org/TR/html4/appendix/notes.html#h-B.5.2.2). The main differences with GridLayout
 * is that it has two passes and that width and height are not calculated in the same pass.
 * <p>
 * The advantage of the algorithm over GridLayout is that it is capable of flowing text controls
 * capable of line wrap. These controls do not have natural 'preferred size'. Instead, they are
 * capable of providing the required height if the width is set. Consequently, this algorithm first
 * calculates the widths that will be assigned to columns, and then passes those widths to the
 * controls to calculate the height. When a composite with this layout is a child of the scrolling
 * composite, they should interact in such a way that reduction in the scrolling composite width
 * results in the reflow and increase of the overall height.
 * <p>
...
public final class TableWrapLayout2 extends Layout implements ILayoutExtension {
	/**
	...
	/**
	 * Bottom margin variable (default is 5).
	 */
	...
	/**
	 * If set to <code>true</code>, all the columns will have the same width. Otherwise, column widths
	 * will be computed based on controls in them and their layout data (default is <code>false</code>
	 * ).
	 */
	...
	private class RowSpan {
		...
		public void update(int currentRow, int rowHeight) {
			TableWrapData2 td = (TableWrapData2) child.getLayoutData();
	...
	protected void layout(Composite parent, boolean changed) {
		...
		m_columnOrigins = new int[numColumns];
		m_rowOrigins = new int[grid.size()];
		int y = topMargin + clientArea.y;
		for (int i = 0; i < grid.size(); i++) {
			int rowHeight = rowHeights[i];
			int x = leftMargin + clientArea.x;
			TableWrapData2[] row = (TableWrapData2[]) grid.elementAt(i);
			for (int j = 0; j < numColumns; j++) {
				m_columnOrigins[j] = x;
				m_rowOrigins[i] = y;
				TableWrapData2 td = row[j];
				if (td.isItemData) {
	...
	int[] computeRowHeights(Control[] children, int[] columnWidths, boolean changed) {
		...
		for (Enumeration enm = rowspans.elements(); enm.hasMoreElements();) {
			...
			TableWrapData2 td = (TableWrapData2) rowspan.child.getLayoutData();
			int ngrowing = 0;
	...
	void placeControl(Control control, TableWrapData2 td, int x, int y, int[] rowHeights, int row) {
		...
		// align horizontally
		if (td.align == TableWrapData2.CENTER) {
			xloc = x + colWidth / 2 - width / 2;
		} else if (td.align == TableWrapData2.RIGHT) {
			xloc = x + colWidth - width;
		} else if (td.align == TableWrapData2.FILL) {
			width = colWidth;
		}
		// align vertically
		if (td.valign == TableWrapData2.MIDDLE) {
			yloc = y + slotHeight / 2 - height / 2;
		} else if (td.valign == TableWrapData2.BOTTOM) {
			yloc = y + slotHeight - height;
		} else if (td.valign == TableWrapData2.FILL) {
			height = slotHeight;
		}
	...
	void createGrid(Composite composite) {
		...
		for (int i = 0; i < children.length; i++) {
			...
			TableWrapData2 spec = (TableWrapData2) child.getLayoutData();
			while (((TableWrapData2[]) grid.elementAt(row))[column] != null) {
				column = column + 1;
			...
			((TableWrapData2[]) grid.elementAt(row))[column] = spec;
			spec.childIndex = i;
			...
			for (int r = 1; r <= rowFill; r++) {
				for (int c = 0; c < spec.colspan; c++) {
					spacerSpec = new TableWrapData2();
					spacerSpec.isItemData = false;
					((TableWrapData2[]) grid.elementAt(row + r))[column + c] = spacerSpec;
				}
			}
		...
		for (int k = column + 1; k < numColumns; k++) {
			spacerSpec = new TableWrapData2();
			spacerSpec.isItemData = false;
			((TableWrapData2[]) grid.elementAt(row))[k] = spacerSpec;
		}
		for (int k = row + 1; k < grid.size(); k++) {
			spacerSpec = new TableWrapData2();
			spacerSpec.isItemData = false;
			((TableWrapData2[]) grid.elementAt(k))[column] = spacerSpec;
		}
	...
	private void updateGrowingColumns(Vector growingColumns, TableWrapData2 spec, int column) {
		...
		growingColumns.add(affectedColumn);
	}

	private void updateGrowingRows(Vector growingRows, TableWrapData2 spec, int row) {
		...
		growingRows.add(affectedRow);
	}

	private TableWrapData2[] createEmptyRow() {
		TableWrapData2[] row = new TableWrapData2[numColumns];
		for (int i = 0; i < numColumns; i++) {
			row[i] = null;
		}
		return row;
			...
			for (int j = 0; j < numColumns; j++) {
				...
				if (width <= current) {
					...
					if (ndiv == 0) {
						...
						columnWidths[j + td.colspan - 1] += width - current;
					} else {
	...
	void initializeLayoutData(Composite composite) {
		...
		for (int i = 0; i < children.length; i++) {
			...
			if (child.getLayoutData() == null) {
				child.setLayoutData(new TableWrapData2());
			}
		}
	}
}
