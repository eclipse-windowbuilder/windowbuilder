...
class ToolkitControl extends Composite {
  ...
  private void initControl() {
    GridLayoutFactory.fillDefaults().numColumns(3).spacing(3, 2).extendedMargins(5, 0, 4, 2).spacing(
        5,
        0).applyTo(this);
    listenTo(this);
    Label iconLabel = new Label(this, SWT.NULL);
    iconLabel.setBackground(getBackground());
    GridDataFactory.swtDefaults().align(SWT.CENTER, SWT.BEGINNING).span(1, 2).indent(0, 2).applyTo(
        iconLabel);
    listenTo(iconLabel);
    ...
    if (wbToolkit.isInstalled()) {
      nameLabel.setText(nameLabel.getText() + Messages.ToolkitControl_installedStatus);
