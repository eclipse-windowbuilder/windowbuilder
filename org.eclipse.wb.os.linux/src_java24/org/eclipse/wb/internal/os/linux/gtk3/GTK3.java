...
public final class GTK3 extends Native {
	private static class InstanceHolder extends NativeHolder {
		...
		static final MethodHandle gtk_widget_get_window = createHandle(GTK, "gtk_widget_get_window",
				FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS));

		static final MethodHandle gtk_widget_hide = createHandle(GTK, "gtk_widget_hide",
				FunctionDescriptor.of(ValueLayout.ADDRESS));

		static final MethodHandle gtk_window_set_keep_above = createHandle(GTK, "gtk_window_set_keep_above",
				FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.JAVA_BOOLEAN));

		static final MethodHandle gtk_widget_show_now = createHandle(GTK, "gtk_widget_show_now",
				FunctionDescriptor.of(ValueLayout.ADDRESS));
	}
	...
	public static GdkWindow gtk_widget_get_window(GtkWidget widget) {
		MemorySegment segment = MemorySegment.ofAddress(widget.handle());
		MemorySegment handle = (MemorySegment) callSafe(() -> InstanceHolder.gtk_widget_get_window.invoke(segment));
		return new GdkWindow(handle.address());
	}
	...
	public static void gtk_widget_show_now(GtkWidget widget) {
		MemorySegment segment = MemorySegment.ofAddress(widget.handle());
		runSafe(() -> InstanceHolder.gtk_widget_show_now.invoke(segment));
	}

	public static void gtk_window_set_keep_above(GtkWidget widget, boolean setting) {
		MemorySegment segment = MemorySegment.ofAddress(widget.handle());
		runSafe(() -> InstanceHolder.gtk_window_set_keep_above.invoke(segment, setting));
	}
}
