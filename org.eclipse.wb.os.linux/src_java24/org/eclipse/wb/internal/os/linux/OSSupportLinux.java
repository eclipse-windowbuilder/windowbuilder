...
public final class OSSupportLinux extends OSSupport {
	private static Version SWT_VERSION_3126 = new Version(3, 126, 0);
	...
	public void beginShot(Control control) {
		...
		if (!isWorkaroundsDisabled()) {
			...
			try {
				...
				if (currentVersion.compareTo(SWT_VERSION_3126) < 0) {
			...
			if (m_eclipseShell != null) {
				GTK3.gtk_window_set_keep_above(new GtkWidget(m_eclipseShell), true);
			}
		}
	...
	public void endShot(Control control) {
		...
		if (!isWorkaroundsDisabled()) {
			GTK3.gtk_widget_hide(new GtkWidget(shell));
			if (m_eclipseShell != null) {
				GTK3.gtk_window_set_keep_above(new GtkWidget(m_eclipseShell), false);
			}
		}
	}
